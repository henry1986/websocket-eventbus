...
class LoggingSettings{
    companion object {
        fun setLogging(loggerFilterList:List<LoggingFilterRule>, globalLevel: KotlinLoggingLevel){
            ...
            KotlinLoggingConfiguration.FORMATTER = object : Formatter {
                ...
                override fun formatMessage(level: KotlinLoggingLevel, loggerName: String, marker: Marker?, t: Throwable?, msg: () -> Any?): Any? {
                    if (loggerFilterList.any { it.loggerName == loggerName && it.isLoggingEnabled(level) } || loggerFilterList.none { it.loggerName == loggerName }) {
                        val message = "${level.name}: [$loggerName] ${marker?.getName()?:""} ${msg()}${t.throwableToString()}"
                        return date(message)
                    }
                    return null
                }
